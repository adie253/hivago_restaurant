                                    ...
                                    {history.map((payout) => (
                                        <tr key={payout.id} className="group hover:bg-slate-50/50 transition-all cursor-pointer" onClick={() => handleViewDetail(payout.id)}>
                                            ...
                                            <td className="px-8 py-6 text-right">
                                                <button className="rounded-xl bg-slate-900 px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-white opacity-0 group-hover:opacity-100 transition-all active:scale-95">
                                                    View Details
                                                </button>
                                            </td>
                                        </tr>
                        ...
                        <div className="flex flex-wrap items-end gap-4 rounded-[32px] bg-white p-8 shadow-sm border border-slate-50">
                            <div className="space-y-2">
                                ...
                                <input 
                                    ...
                                    onChange={(e) => setTaxDateRange(prev => ({ ...prev, from: e.target.value }))}
                            ...
                            <div className="space-y-2">
                                ...
                                <input 
                                    ...
                                    onChange={(e) => setTaxDateRange(prev => ({ ...prev, to: e.target.value }))}
                                    className="block rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-900 focus:border-[#AD221F] outline-none transition-all"
                                />
                            </div>
                            <button 
                                ...
                                <div className="rounded-[32px] bg-white p-8 shadow-sm border border-slate-50">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center p-4 rounded-2xl bg-emerald-50/50">
                                            ...
                                            <span className="text-lg font-bold text-emerald-900">{formatCurrency(gstSummary.totalGstOnOrders)}</span>
                                        </div>
                                        <div className="flex justify-between items-center p-4 rounded-2xl bg-slate-50">
                                            ...
                                            <span className="text-lg font-bold text-slate-900">{formatCurrency(gstSummary.totalCommissionGst)}</span>
                                        </div>
                                        <div className="pt-4 border-t border-slate-50 flex justify-between">
                                            <span className="text-xs font-bold text-slate-400">{gstSummary.orderCount} orders processed</span>
                                            <button className="text-xs font-bold text-[#AD221F] uppercase tracking-widest hover:underline">Download CSV</button>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="rounded-[32px] bg-white p-8 shadow-sm border border-slate-50">
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-center p-4 rounded-2xl bg-orange-50/50">
                                                ...
                                                <span className="text-lg font-bold text-orange-900">{formatCurrency(tdsSummary.totalTdsDeducted)}</span>
                                            </div>
                                            <div className="flex justify-between items-center p-4 rounded-2xl bg-slate-50">
                                                <span className="text-sm font-bold text-slate-600">Net Receivable after TDS</span>
                                                <span className="text-lg font-bold text-slate-900">{formatCurrency(tdsSummary.netAfterTds)}</span>
                                            </div>
                                            <div className="pt-4 border-t border-slate-50 flex justify-between">
                                                <span className="text-xs font-bold text-slate-400">For {tdsSummary.grossOrderAmount.toLocaleString()} gross sales</span>
                                                <button className="text-xs font-bold text-[#AD221F] uppercase tracking-widest hover:underline">Download CSV</button>
                                            </div>
                                        </div>
                                    </div>
